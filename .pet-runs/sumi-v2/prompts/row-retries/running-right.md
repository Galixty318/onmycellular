Create Codex pet row `running-right` for `sumi`: exactly 8 full-body frames in one horizontal strip on flat pure cyan #00FFFF.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: Preserve the existing tiny sumi-e ink-brush bird identity, expressive black brushwork, white negative-space body, and small red blossom accents.. Preserve silhouette, face, palette, material, proportions, markings, and props.
